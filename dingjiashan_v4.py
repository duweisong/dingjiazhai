...
===============================
V3问题: 回撤37%, 波动过大
V4目标: 回撤<15%, 通过组合级风控+紧止损+现金储备实现
核心改动:
  1. 组合回撤断路器: 峰值回撤>12%减至1只, >15%清仓
  2. 现金储备: 始终保留15%现金
  3. 紧止损: -6%硬止损, 1.0x ATR低波移动止损
  4. 缩仓: 强牛4只/中性2只/熊市1只
  5. 提高门槛: 评分<0.5不买, 需MA5>MA10
  6. 缩短周期: 持仓10天, 止盈+12%
  7. 单只上限: 不超过总资金25%
"""
...
MIN_SCORE = 0.3
CASH_RESERVE = 0.35
MAX_SINGLE_PCT = 0.12     # 中性/强牛单只12%
MAX_SINGLE_BEAR = 0.08    # 熊市单只8%
MAX_SECTOR = 2
DAILY_DD_CUT = -0.03
...
# V4b: 适度紧止损 + 分散化
EXIT_CONFIG = {
    'atr_mult_low': 1.2,     # 恢复V3
    'atr_mult_high': 2.0,    # 恢复V3
    'max_hold_days': 12,
    'profit_target': 0.15,
    'ma_period': 20,
    'rank_exit': 80,
    'rank_drop_exit': 50,
    'stop_loss_hard': -0.05,  # 回到-5%
    'stop_loss_bear': -0.03,
}
...
def compute_factors(df, date_str):
    ...
    # V4: MA5>MA10 requirement for buy
    ma5_gt_ma10 = 1.0 if ma_5 > ma_10 else 0.0
    tr_list=[]
    ...
    return {'ret_5d':ret_5d,'ret_10d':ret_10d,'ret_20d':ret_20d,
        ...
        'turnover':1.0,'up_days_10':up_days_10,'ret_overbought':0,
        'close':close[-1],'atr_20':atr_20,'ma_20':ma_20,'ma_60':ma_60,
        'avg_amount':avg_amount,'ma5_gt_ma10':ma5_gt_ma10}
...
def score_stocks(all_data, date_str):
    ...
    # V4: MA5>MA10 as soft factor, not hard filter (removed hard filter)
...
class Portfolio:
    def __init__(self):
        ...
        self.peak_equity=INITIAL_CAPITAL  # V4: track peak for DD
        self.sectors=Counter()

    def max_pos(self):
        if self.is_bear: return POS_BEAR
        if self.is_strong: return POS_STRONG
        return POS_NEUTRAL

    def portfolio_dd(self):
        """Current drawdown from peak"""
        total = self.cash
        # approximate - will be updated in mark()
        return (total / self.peak_equity - 1) if self.peak_equity > 0 else 0
    ...
    def check_exit(self, pos, close, ma20, rank, days_held, atr_pct):
        ...
        # V4: dynamic trail always uses tighter multipliers
    ...
    def buy(self,code,name,price,score,rank,date_str,atr,atr_pct):
        ...
        # V4: sector limit
        sec=get_sector(code)
        if self.sectors.get(sec,0)>=MAX_SECTOR: return False
        # V4: check if we have enough cash reserve
        reserve_target = INITIAL_CAPITAL * CASH_RESERVE

        total_eq=self.cash
        for c,p in self.positions.items(): total_eq+=p['shares']*p['entry_price']
        max_p=self.max_pos()

        base=total_eq/max(max_p,1)
        # Cap single position (tighter in bear)
        single_cap = MAX_SINGLE_BEAR if self.is_bear else MAX_SINGLE_PCT
        base = min(base, total_eq * single_cap)

        if atr_pct>5: va=0.5
        elif atr_pct>3: va=0.7
        else: va=0.9

        target=base*va
        # V4: ensure cash reserve
        max_spend = self.cash - reserve_target
        if target > max_spend: target = max(0, max_spend)
        ...
        if cost>self.cash or (self.cash-cost)<reserve_target*0.5: return False
        self.cash-=cost
    ...
    def force_reduce(self, all_data, date_str, reason):
        """V4: 回撤超限强制清仓至1只"""
        if len(self.positions) <= 1: return
        # Sort by profit, keep best, sell rest
        held = []
        for code, pos in self.positions.items():
            if code in all_data:
                d = all_data[code]['df']; d = d[d['date'] <= date_str]
                if len(d) > 0:
                    pnl = (d['close'].iloc[-1] / pos['entry_price'] - 1)
                    held.append((pnl, code, d['close'].iloc[-1]))
        held.sort(reverse=True)
        for pnl, code, price in held[1:]:  # sell all except best
            self.sell(code, price, f'ForceReduce_{reason}', date_str)
...
def run_backtest(all_data):
    ...
    print(f'\n=== 丁家山 V4: {dates[0]} ~ {dates[-1]} ({len(dates)} days) ===\n')
    ...
    for idx,date_str in enumerate(dates):
        if bench_ma is not None:
            ...
            if dt in bench_ma.index:
                ...
                if len(ic)>0:
                    ...
                    pf.is_strong=idx_close>ma_val*1.05
        ...
        # V4: Force reduce if portfolio DD too high
        total_now = pf.cash
        for code, pos in pf.positions.items():
            if code in all_data:
                d = all_data[code]['df']; d = d[d['date'] <= date_str]
                if len(d) > 0: total_now += pos['shares'] * d['close'].iloc[-1]
        pf.peak_equity = max(pf.peak_equity, total_now)
        dd_now = (total_now / pf.peak_equity - 1) if pf.peak_equity > 0 else 0
        ...
        if slots>0 and not skip_buy:
            ...
            for _,row in cand.iterrows():
                if slots<=0: break
                code=row['code']
                if code in info_map and float(row['score']) >= MIN_SCORE:
        ...
        # V4e: 单日暴跌断路器
        daily_chg = (total / prev_equity - 1) if prev_equity > 0 else 0
        if daily_chg <= DAILY_DD_CUT and len(pf.positions) >= 2:
            held_pnl = []
            for code, pos in pf.positions.items():
                if code in info_map:
                    pnl = (info_map[code]['close'] / pos['entry_price'] - 1)
                    held_pnl.append((pnl, code))
            held_pnl.sort()
            for pnl, code in held_pnl[:len(held_pnl)//2]:
                pf.sell(code, info_map[code]['close'], 'DailyCut', date_str)
                sells += 1
        prev_equity = total

        if (idx+1)%50==0 or idx==0:
            ret=(total/INITIAL_CAPITAL-1)*100
            dd=(total/pf.peak_equity-1)*100 if pf.peak_equity>0 else 0
            regime='BEAR' if pf.is_bear else ('STRONG' if pf.is_strong else 'NEUT')
            print(f'  [{date_str}] D{idx+1}/{len(dates)} {regime} | '
                  f'Pos={len(pf.positions)}/{pf.max_pos()} | Val={total:,.0f} | '
                  f'Ret={ret:+.2f}% | DD={dd:+.1f}%')
    ...
    print(f'\n{"="*60}')
    print(f'  丁家山 V4 RESULTS')
    print(f'{"="*60}')
    print(f'  Total Return:   {total_ret:+.2f}%')
    print(f'  Annual Return:  {ann_ret:+.2f}%')
    print(f'  Sharpe Ratio:   {sharpe:.2f}')
    print(f'  Max Drawdown:   {max_dd:.2f}%', '** TARGET <15% **' if abs(max_dd)<=15 else 'MISSED TARGET')
    ...
    result={'strategy':'丁家山_V4','dates':pf.dates,'equity':pf.equity,'trades':pf.trades,
    ...
    pickle.dump(result,open(os.path.join(CACHE_DIR,'dingjiashan_v4_result.pkl'),'wb'))
    return result
